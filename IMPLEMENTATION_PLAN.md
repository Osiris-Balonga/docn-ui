...
Date: 2026-08-28. Status: the V1 sequence is complete and released through L16. On 2026-09-12 the maintainer requested a sequential post-V1 integration program, L17 through L22, with 49 planned stories. The [lot status](docs/implementation/status.json) records actual progress; `currentLot` is L18; L17 is merged and L18 is verified locally and in review through PR #86, pending an authorized merge. Language revision: 2026-08-29; all project content must be in English, while conversation with the maintainer is in French. Product-direction clarification: 2026-08-30; docn-ui extends an existing shadcn project through an additional registry and does not introduce a parallel project configuration. Component revision: 2026-08-31; L12-S02 was split into sequential stories to cover PDFx's component categories, barcodes, individual source installation and usable documentation. Post-V1 direction: preserve the qualified document guarantees while making the normal template API materially simpler than the current low-level render path.
...
- **G3 / L07**: source installed through the official CLI in an existing shadcn configuration, including a non-default import prefix, without dependence on the monorepo or docn-ui site.
...
The maintainer requires GitHub before L01 and confirms a public `Osiris-Balonga/docn-ui` repository. L00G setup is authorized; merging is a separate decision. License, host, domain, and site publication are undecided. Unavailable GitHub access blocks L00G; it does not permit skipping it or starting L01. A user-configured `@docn` mapping is the planned registry ergonomics, but official registry-directory submission, npm publication, hosting, and license selection remain unauthorized external actions.
