...
try {
  if (process.env.GITHUB_EVENT_NAME !== 'pull_request_target') {
    throw new Error('The branch policy must run from the trusted target context.');
  }
  ...
  console.error('Unable to verify pull request metadata from the trusted target context.');
  process.exitCode = 1;
}
