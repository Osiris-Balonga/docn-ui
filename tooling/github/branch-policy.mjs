...
export function evaluateBranchPolicy(event) {
  ...
  if (base.ref === 'main') {
    return sameRepository && head.ref === 'dev'
      ...
      : { allowed: false, reason: 'Only dev from this repository may target main.' };
  }

  if (base.ref === 'dev') {
    const workingBranch = /^(feat|fix|chore|docs|test|ci|build|refactor|release)\/[^\s]+$/.test(head.ref);
    return workingBranch || (sameRepository && head.ref === 'main')
      ? { allowed: true, reason: 'Work branch or same-repository main synchronization.' }
      : { allowed: false, reason: 'Use a documented working branch to target dev.' };
  }
