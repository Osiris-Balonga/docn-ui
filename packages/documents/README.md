...
- S02 adds a non-React template-contract layer above `core` and `themes`. It
  owns `TemplateDescriptor<TData>`, validates the discriminated public format
  input and safe theme envelope, and purely normalizes already-preflighted
  local-image descriptors. `core` gains no dependency on themes or assets.
...
- Browser workers resolve a template from a static trusted ID map. A
  ...
  resolver never crosses `postMessage`.
