...
### Activation status through L06

`unit`, `components`, and `integration` are configured with exclusive globs. Components contains one project-link composition suite. Integration contains one worker-queue suite. No pure document logic exists yet, so unit is **not a verified suite** and its standalone command fails with "No test files found"; `passWithNoTests` remains false. The shared lightweight command selects all three projects and collects each existing file once.

The `pdf` scope is active with one real feasibility suite covering renderer output, dimensions, text, sides, page boxes, pagination, four receipt outputs, and a physical-limit error. Its five cases represent distinct engine risks and share the same fixtures and readers. The `integration` scope injects a small worker port for latest-revision replacement, a worker error, and destruction; it does not duplicate PDF geometry. Consumers, E2E, visual, and asset/registry/bundle verification commands are not implemented yet. Their file conventions are reserved and excluded from the lightweight projects. Do not add empty successful placeholder scripts.

Available now: `test`, the three lightweight scoped commands, `test:pdf`, `test:e2e`, `test:watch`, `test:coverage`, `test:all`, `quality`, `validate`, and `validate:full`. Unit coverage targets formats/contracts, themes/assets, image envelopes, and fixed-frame measurement/boundaries. Components cover site compositions and the business-card editor with a simulated render session; integration covers worker revision coordination; the PDF scope contains the shared feasibility file and business-card family file. Two E2E files use Playwright Chromium against an isolated static server: one catalog discovery journey and one real worker/PDF playground journey. `test:all --list` prints activation without executing tests. The sequential orchestrator reads the actual package scripts; adding a heavy command activates it automatically. A root build records the source SHA and input hash. `test:all` reuses that build only when the fingerprint verifies, while standalone E2E creates its own isolated build.
