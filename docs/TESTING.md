...
Available now: `test`, the three lightweight scoped commands, `test:pdf`, `test:e2e`, `test:watch`, `test:coverage`, `test:all`, `quality`, `validate`, and `validate:full`. Unit coverage targets formats/contracts, themes/assets, and fixed-frame measurement/boundaries. Components cover site compositions and the business-card editor with a simulated renderer; integration covers worker revision coordination; the PDF scope contains the shared feasibility file and business-card family file. The single E2E file uses Playwright Chromium against an isolated built static server for the real edit, back-side preview, and export journey. `test:all --list` prints activation without executing tests. The sequential orchestrator reads the actual package scripts; adding a heavy command activates it automatically. `validate:full` still builds before `test:all`, while standalone E2E performs its own isolated build; reusable build fingerprinting/artifact handoff remains planned for L06.
