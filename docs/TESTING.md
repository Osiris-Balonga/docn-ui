...
Available now: `test`, the three lightweight scoped commands, `test:pdf`, `test:watch`, `test:coverage`, `test:all`, `quality`, `validate`, and `validate:full`. Unit coverage currently targets formats/contracts, themes/assets, and fixed-frame measurement/boundaries. Components cover site composition; integration covers worker revision coordination; the separate PDF scope contains the shared feasibility file. `test:all --list` prints activation without executing tests. The sequential orchestrator reads the actual package scripts; adding a heavy command activates it automatically. `validate:full` prepares one build and runs each activated scope once. Build fingerprinting/artifact handoff will be implemented with the first E2E suite in L06; no reusable E2E artifact is claimed today.
