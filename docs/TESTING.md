...
### Activation status through L07-S01

`unit`, `components`, and `integration` are configured with exclusive globs and all three scopes are active. Components contains site composition and editor suites. Integration contains the worker-queue suite. Unit contains document contracts, layout utilities, themes/assets, QR behavior, and the registry graph/transformation suite. `passWithNoTests` remains false, and the shared lightweight command selects all three projects and collects each existing file once.

The `pdf` scope is active with one real feasibility suite covering renderer output, dimensions, text, sides, page boxes, pagination, four receipt outputs, and a physical-limit error. Its five cases represent distinct engine risks and share the same fixtures and readers. The `integration` scope injects a small worker port for latest-revision replacement, a worker error, and destruction; it does not duplicate PDF geometry. E2E and registry verification are active. Consumers, visual, asset verification, and bundle verification are not implemented yet. Their file conventions are reserved and excluded from the lightweight projects. Do not add empty successful placeholder scripts.

Available now: `test`, the three lightweight scoped commands, `test:pdf`, `test:e2e`, `test:watch`, `test:coverage`, `test:all`, `quality`, `validate`, `validate:full`, `generate:registry`, and `verify:registry`. Unit coverage targets formats/contracts, themes/assets, image envelopes, fixed-frame measurement/boundaries, and registry graph/import transformations. Components cover site compositions and the business-card editor with a simulated render session; integration covers worker revision coordination; the PDF scope contains the shared feasibility file and business-card family file. Two E2E files use Playwright Chromium against an isolated static server: one catalog discovery journey and one real worker/PDF playground journey. `test:all --list` prints activation without executing tests. The sequential orchestrator reads the actual package scripts; adding a heavy command activates it automatically. A root build generates and validates the registry, then records the source SHA and input hash. `test:all` reuses that build only when the fingerprint verifies, while standalone E2E creates its own isolated build.
...
CI through L06 has one lightweight Vitest job, one PDF job, one build job, and one dependent Chromium job. The build job uploads `apps/www/out` with its fingerprint; Chromium verifies and serves that exact artifact without running the lightweight or PDF suites again. Consumer and visual jobs activate only with their first distinct coverage. Conditional jobs explicitly report `not-applicable`, never an invented result; release runs every activated scope.
