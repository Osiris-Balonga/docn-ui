...
All geometry is native PDF SVG, with selectable labels, theme fonts and grayscale defaults derived from existing PDF theme roles. The title, numeric ticks, category labels, Cartesian values and series legend are always visible. Circular sectors have numbered leader labels and matching numbered category/value/share legends, so identification does not rely on color alone. Label leaders on each side are ordered and spaced independently to prevent dense sectors from colliding. There are no hover-only values, screenshots, remote renderers or site chart dependencies.
