# Versioned registry installation

The immutable v1.0.0 registry can be inspected from a local static build before using its public origin.
...
- A static docn-ui build served with `corepack pnpm preview` on port 4173. The separate `corepack pnpm dev` workflow uses the mutable development channel on the origin printed by Next.js.
...
```sh
corepack pnpm dlx shadcn@4.19.1 add http://127.0.0.1:4173/r/v1.0.0/docn-business-card-violet-founder.json
```
...
The direct URL is the qualified local path. At the approved public HTTPS origin, add docn-ui alongside other registries in the same `components.json`:

```json
{
  "registries": {
    "@docn": "https://docn-ui.vercel.app/r/v1.0.0/{name}.json"
  }
}
```

Then the official CLI can install `@docn/docn-business-card-violet-founder`. Use the public command only after the immutable release path has been deployed and verified.
...
```sh
corepack pnpm dlx shadcn@4.19.1 view http://127.0.0.1:4173/r/v1.0.0/docn-business-card-violet-founder.json
corepack pnpm dlx shadcn@4.19.1 add http://127.0.0.1:4173/r/v1.0.0/docn-business-card-violet-founder.json --dry-run
corepack pnpm dlx shadcn@4.19.1 add http://127.0.0.1:4173/r/v1.0.0/docn-business-card-violet-founder.json --diff
```
