...
Fonts and document images use separate channels. Font assets remain fixed manifest entries prepared by the existing asset installer. Serializable template data contains only bounded local image IDs. After data validation, IDs are canonicalized, made unique, limited to two and sorted. A `runtimeOptions` resolver returns bytes and an untrusted declared PNG/JPEG MIME type to `preflightLocalImages`; preflight independently sniffs and decodes the bytes and produces `PreparedLocalImages` with pure `{ id, mimeType, byteLength, widthPx, heightPx, sha256 }` descriptors. The pure normalizer receives the exact descriptor set, never bytes or a resolver, and fingerprints it. L18 worker messages transfer validated `ArrayBuffer`s separately from serializable input and revoke temporary URLs. Neither data nor worker requests contain arbitrary URLs or filesystem paths.
...
Normalization receives exact `FontManifestIdentity` and local-image descriptor values, never resolvers. The frozen resolved theme supplies the preset identity, colors, base-preserving weights/type scale/spacing, and only template-qualified family choices. `TemplatePlanContext` also carries a separate legacy-style projection: legacy adapters may map its colors and qualified family changes into source-owned style props, but never weights, type scale, spacing, or geometry. A descriptor declares supported print-profile kinds and a compatible default; the L17 continuous feasibility descriptor is `screen`-only and rejects `print` before plan creation.
